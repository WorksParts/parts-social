...
function worksparts_social_language_attributes( $attributes ) {
...
add_filter( 'language_attributes', 'worksparts_social_language_attributes' );

function worksparts_fb_root() {
...
add_action( 'worksparts_before_body', 'worksparts_fb_root' );

function worksparts_social_button_conf( $post_id = '' ) {
	if ( !$post_id && !is_404() )
		$post_id = get_the_ID();
	...
	$value         = array(
		...
		'counturl'     => worksparts_get_summary_permalink( $post_id ),
		'url'          => worksparts_get_summary_permalink( $post_id ),
...
function worksparts_global_social_button() {
	echo worksparts_get_global_social_button();
}
function worksparts_get_global_social_button() {
	$twitter_url   = esc_attr( get_option( 'twitter_url' ) );
	$facebook_url  = esc_attr( get_option( 'facebook_url' ) );
	$youtube_url   = esc_attr( get_option( 'youtube_url' ) );
	$pinterest_url = esc_attr( get_option( 'pinterest_url' ) );
...
function worksparts_social_button( $post_id = '' ) {
	echo worksparts_get_social_button( $post_id );
}
function worksparts_get_social_button( $post_id = '' ) {
	$output = '';
	if ( !$post_id && !is_404() )
		$post_id = get_the_ID();

	$conf = worksparts_social_button_conf( $post_id );
	extract($conf);
...
function worksparts_mobile_header_social_button() {
	echo worksparts_get_mobile_header_social_button();
}
function worksparts_get_mobile_header_social_button() {
